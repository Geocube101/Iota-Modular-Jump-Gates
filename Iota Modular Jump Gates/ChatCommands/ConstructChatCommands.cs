	...
	internal class MyConstructInfoChatCommand : MyChatCommand
	{
		...
		private string GenerateConstructInfo(ulong caller, MyJumpGateConstruct construct)
		{
			...
			return MyTexts.GetString("ChatCommandHandler_GridInfoCommand_GridInfo")
				...
				.Replace("{%3}", construct.HasCommLink().ToString())
				.Replace("{%4}", construct.GetCommLinkedJumpGateGrids().Count().ToString())
				.Replace("{%5}", construct.GetBeaconsWithinReverseBroadcastSphere().Count().ToString())
				.Replace("{%6}", MyJumpGateModSession.AutoconvertMetricUnits(construct.ConstructMass() * 1000, "g", 4))
				.Replace("{%7}", (admin) ? Vector3D.Round(construct.ConstructVolumeCenter(), 2).ToString() : "//////")
				.Replace("{%8}", (admin) ? Vector3D.Round(construct.ConstructMassCenter(), 2).ToString() : "//////")
				.Replace("{%9}", (construct.GetCubeGridPhysics() != null).ToString())
				.Replace("{%10}", construct.GetAttachedJumpGateControllers().Count().ToString())
				.Replace("{%11}", construct.GetAttachedJumpGateDrives().Count().ToString())
				.Replace("{%12}", construct.GetAttachedJumpGateCapacitors().Count().ToString())
				.Replace("{%13}", construct.GetAttachedJumpGateRemoteAntennas().Count().ToString())
				.Replace("{%14}", construct.GetAttachedJumpGateServerAntennas().Count().ToString())
				.Replace("{%15}", construct.GetAttachedJumpGateRemoteLinks().Count().ToString())
				.Replace("{%16}", construct.GetAttachedLaserAntennas().Count().ToString())
				.Replace("{%17}", construct.GetAttachedRadioAntennas().Count().ToString())
				.Replace("{%18}", construct.GetAttachedBeacons().Count().ToString())
				.Replace("{%19}", construct.GetJumpGates().Count().ToString())
				.Replace("{%20}", gridcount.ToString())
				.Replace("{%21}", staticcount.ToString())
