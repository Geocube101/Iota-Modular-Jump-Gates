...
{
	public enum MyPacketTypeEnum { DOWNLOAD_GRID, UPDATE_GRIDS, UPDATE_GRID, UPDATE_JUMP_GATE, UPDATE_DRIVE, UPDATE_CAPACITOR, UPDATE_CONTROLLER, UPDATE_JUMP_ENDPOINT, CLOSE_BLOCK, CLOSE_GRID, JUMP_GATE_JUMP, JUMP_GATE_VOID_IN, JUMP_GATE_VOID_OUT, STATICIFY_CONSTRUCT, COMM_LINKED, BEACON_LINKED, AUTOACTIVATE, SHEARWARNING };
			...
			else if (MyNetworkInterface.IsMultiplayerServer)
			{
				...
				if (callbacks != null)
				{
					...
					for (int i = 0; i < callbacks_local.Count; ++i)
					{
						...
						{
							Logger.Error($"\"{e.GetType().FullName}\" during packet callback[{i}]\n -> {e.Message}\nStackTrace:\n  ...\n{e.StackTrace}\nInnerStackTrace:\n  ...\n{e.InnerException?.StackTrace}");
						}
					}
				}
			...
			else if (from_server)
			{
				...
				if (callbacks != null)
				{
					...
					for (int i = 0; i < callbacks_local.Count; ++i)
					{
						...
						{
							Logger.Error($"\"{e.GetType()?.FullName ?? "UNKNOWN"}\" during packet callback[{i}]\n -> {e.Message}\nStackTrace:\n  ...\n{e.StackTrace}\nInnerStackTrace:\n  ...\n{e.InnerException?.StackTrace ?? "???"}");
						}
					}
				}
			}
